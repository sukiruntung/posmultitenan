...
class ValidatePenerimaanBarang extends EditRecord
{
    ...
    {
        return 'Validasi Penerimaan Barang';
    }
        ...
        DB::transaction(function () use ($data) {
            foreach ($this->products as $detail) {
                ...
                $cekHistory = ProductStockHistories::where('product_stock_id', $productStock->id)
                    ->where('no_transaksi', $data['penerimaan_barang_invoicenumber'])
                    ->exists();
                if ($cekHistory) {
                    Notification::make()
                ...
                $supplierProduct = SupplierProduct::updateOrCreate(
                    ...
                    [
                        'first_product_stock_id' => $productStock['id'],
                        'harga_beli'             => $detail['harga'],
                    ]
                );
                $supplierProduct->increment('frekuensi');
            }
