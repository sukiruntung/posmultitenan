...
trait HasMenuAccess
{
    public function cacheMenuAccess()
    {
        ...
        $cacheKeyHeader = "menu_access_ids:{$this->user_group_id}:outlet_id:{$user->userOutlet->outlet_id}";
        ...
        foreach ($menuAccesses as $access) {
            $cacheKey = "menu_access:{$this->user_group_id}:outlet_id:{$user->userOutlet->outlet_id}:{$access->menu_id}";
    ...
    public function getCachedMenuAccess(int $menuId)
    {
        ...
        return Cache::get("menu_access:{$this->user_group_id}:outlet_id:{$user->userOutlet->outlet_id}:{$menuId}");
    }
